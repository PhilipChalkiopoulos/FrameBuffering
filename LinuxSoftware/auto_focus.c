...
void Focus_Init(void){
  // please look at TERASIC_AUTO_FOCUS IP to see register's detail.
  IOWR(h2p_lw_auto_focus,REG_CTRL, 0);// focus mode : 1: window-screen, 0: full-screen
  IOWR(h2p_lw_auto_focus,REG_FOCUS_W, focus_width);// focus_width
  IOWR(h2p_lw_auto_focus,REG_FOCUS_H, focus_height);// focus_height
  IOWR(h2p_lw_auto_focus,REG_FOCUS_X_START, video_w/2-focus_width/2);//x_start
  IOWR(h2p_lw_auto_focus,REG_FOCUS_Y_START, video_h/2-focus_height/2);// y_start

  IOWR(h2p_lw_auto_focus,REG_SCAL, focus_scal*256 + focus_scal_f); // scan 0 -> 1023 , step: SCAL , to find STEP_UP
                                                                          //  scan STEP_UP + - SCAL/2 , step: SCAL_F
  IOWR(h2p_lw_auto_focus,REG_TH, focus_th);

  //////////// focus at initial time
  usleep(100);
  IOWR(h2p_lw_auto_focus,REG_GO, 1);
  usleep(2);
  IOWR(h2p_lw_auto_focus,REG_GO, 0);
}
...
alt_u16 Focus_Window(int x,int y){
   ...
   if(Focus_Released()) { // pre focus done
	   IOWR(h2p_lw_auto_focus,REG_CTRL, 1);// focus mode : 1: window-screen, 0: full-screen
	   ...
	   IOWR(h2p_lw_auto_focus,REG_FOCUS_X_START, x_start);//x_start
	   IOWR(h2p_lw_auto_focus,REG_FOCUS_Y_START, y_start);//y_start
	   ...
	   end_focus = IORD(h2p_lw_auto_focus,REG_STATUS)&0x0FFF;
	   printf("end_focus = %d \n",end_focus);
...
/* 
  ...
  while((IORD(h2p_lw_auto_focus,REG_STATUS)&0x8000) ==0 && alt_nticks() < TimeOut ); // waiting for VCM release I2C bus
